...
if (Meteor.isServer) {
  describe('notes', function() {
    ...
    it('should throw error if extra updates', function() {
      expect(() => {
        Meteor.server.method_handlers['notes.update'].apply({ userId: noteOne.userId }, [noteOne._id, { title: 'new title', name: 'Blah blah' } ]);
      }).toThrow();
    });
    ...
    it('should return only users notes', function() {
    ...
    it('should return zero notes for user that has none', function() {
      const res = Meteor.server.publish_handlers.notes.apply({ userId: 'abc123' });
      const notes = res.fetch();
